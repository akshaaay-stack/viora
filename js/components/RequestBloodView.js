 ...
 * Emergency Blood Request Creator + Live Match Dispatcher & Requester Tracker
 */

import { VIORA_CONFIG } from '../config.js';
import { KERALA_DISTRICT_DATA } from '../keralaData.js';
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-12">
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 pb-6">
        ...
        <div class="flex items-center gap-3">
          <button onclick="window.vioraApp.navigateTo('requests')" class="px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold transition flex items-center gap-1.5">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div class="lg:col-span-7 bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-8">
          ...
          <form id="bloodRequestForm" onsubmit="window.vioraApp.handleCreateRequest(event)" class="space-y-6">
            ...
            <!-- Submit Button -->
            <div class="pt-2">
              <button type="submit" id="submitRequestBtn" class="w-full py-4 rounded-xl bg-rose-700 hover:bg-rose-800 text-white font-bold text-base shadow-lg shadow-rose-900/20 hover:shadow-rose-900/30 flex items-center justify-center gap-2 transition">
            ...
            ` : activeRequests.map(req => {
              ...
              return `
                <div class="p-5 rounded-2xl bg-white border ${isUrgent ? 'border-rose-300 ring-1 ring-rose-200' : 'border-slate-200'} shadow-sm space-y-4">
                      ...
                      return `
                        <div class="p-3 rounded-xl ${isAccepted ? 'bg-emerald-50/80 border border-emerald-200' : 'bg-slate-50 border border-slate-200'} space-y-2">
                          ...
                          <div class="pt-1.5 border-t border-slate-200/60 flex items-center justify-between text-xs">
                            ${isAccepted ? `
                              <button onclick="window.vioraApp.revealContact('${m.id}')" class="px-3 py-1 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs flex items-center gap-1.5 transition">
                  ...
                  ${acceptedMatch ? `
                    <div class="pt-2">
                      <button onclick="window.vioraApp.handleFulfillRequest('${req.id}')" class="w-full py-2.5 rounded-xl bg-slate-900 hover:bg-black text-white text-xs font-bold transition flex items-center justify-center gap-1.5">
