...
export function renderDonorDashboardView() {
  ...
  if (!currentDonor) {
    return `
      <div class="max-w-4xl mx-auto px-4 py-16 text-center">
        ...
        <button onclick="window.vioraApp.openModal({ type: 'donorRegister' })" class="mt-4 px-6 py-2.5 rounded-xl bg-rose-700 text-white font-bold text-sm btn-touch">
          Register as Donor
        </button>
      </div>
    `;
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        ...
        <div class="flex items-center gap-3">
          ...
          <button onclick="window.vioraApp.openModal({ type: 'donorRegister' })" class="px-3 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold transition flex items-center gap-1 btn-touch">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          ...
          <div class="space-y-3 max-w-2xl">
            ...
            <div class="pt-2 flex flex-wrap items-center gap-4 text-xs text-slate-300">
              ...
              <div class="flex items-center gap-1.5">
                ...
                <span>Mandatory Safe Interval: <strong>${CONFIG.MIN_DONATION_INTERVAL_DAYS} Days</strong></span>
              </div>
            </div>
          </div>
          ...
          <div class="bg-white/10 backdrop-blur-md p-5 rounded-2xl border border-white/10 text-center sm:text-left space-y-3 min-w-[240px]">
            ...
            <div class="flex items-center justify-between gap-3">
              ...
              <button onclick="window.vioraApp.handleToggleAvailability('${currentDonor.id}', ${!currentDonor.available})" class="px-4 py-2 rounded-xl text-xs font-bold transition shadow-sm ${currentDonor.available ? 'bg-emerald-500 hover:bg-emerald-600 text-white' : 'bg-slate-700 hover:bg-slate-600 text-slate-200'} btn-touch">
              ...
              return `
                <div class="p-6 rounded-2xl bg-white border ${isAccepted ? 'border-emerald-300 ring-1 ring-emerald-200' : 'border-slate-200'} shadow-sm space-y-4">
                  ...
                  <div class="pt-2 flex items-center gap-3">
                    ${isAccepted ? `
                      <button onclick="window.vioraApp.revealContact('${currentMatch.id}')" class="w-full py-3 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs flex items-center justify-center gap-2 transition shadow-sm btn-touch">
                    ...
                    ` : `
                      <button onclick="window.vioraApp.handleAcceptMatch('${currentMatch?.id}', '${currentDonor.id}')" class="flex-1 py-3 rounded-xl bg-rose-700 hover:bg-rose-800 text-white font-bold text-xs flex items-center justify-center gap-1.5 transition shadow-md shadow-rose-900/10 btn-touch">
                        <i data-lucide="check" class="w-4 h-4"></i>
                        <span>ACCEPT REQUEST</span>
                      </button>
                      <button onclick="window.vioraApp.handleDeclineMatch('${currentMatch?.id}', '${currentDonor.id}')" class="px-4 py-3 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs transition btn-touch">
                        <span>Decline</span>
                      </button>
                    `}
                  </div>
